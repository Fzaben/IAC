...
namespace MyCompanyName.AbpZeroTemplate.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !AbpZeroTemplateConsts.AllowTenantsToChangeEmailSettings) return;
        ...
        {
            if (_multiTenancyConfig.IsEnabled &&
                !await IsTwoFactorLoginEnabledForApplicationAsync()
            ) //Two factor login can not be used by tenants if disabled by the host
                return;
