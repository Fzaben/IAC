...
namespace MyCompanyName.AbpZeroTemplate.Authorization.Users
{
    ...
    public class UserEmailer : AbpZeroTemplateServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public virtual async Task SendEmailActivationLinkAsync(User user, string link, string plainPassword = null)
        {
            ...
            if (user.TenantId.HasValue) link = link.Replace("{tenantId}", user.TenantId.ToString());
            ...
            try
            {
                using (_unitOfWorkManager.Begin())
                {
                    using (_unitOfWorkManager.Current.SetTenantId(tenantId))
                    {
                        ...
                        if (tenantAdmin == null || string.IsNullOrEmpty(tenantAdmin.EmailAddress)) return;
        ...
        public async Task TryToSendFailedSubscriptionTerminationsEmail(List<string> failedTenancyNames, DateTime utcNow)
        {
            try
            {
                ...
                if (hostAdmin == null || string.IsNullOrEmpty(hostAdmin.EmailAddress)) return;
