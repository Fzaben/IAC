    ...
    public static class WebContentDirectoryFinder
    {
        public static string CalculateContentRootFolder()
        {
            var coreAssemblyDirectoryPath =
                Path.GetDirectoryName(typeof(AbpZeroTemplateCoreModule).GetAssembly().Location);
            if (coreAssemblyDirectoryPath == null)
                throw new Exception("Could not find location of MyCompanyName.AbpZeroTemplate.Core assembly!");

            var directoryInfo = new DirectoryInfo(coreAssemblyDirectoryPath);
            while (!DirectoryContains(directoryInfo.FullName, "MyCompanyName.AbpZeroTemplate.Web.sln"))
            {
            ...
            var webMvcFolder = Path.Combine(directoryInfo.FullName,
                $"src{Path.DirectorySeparatorChar}MyCompanyName.AbpZeroTemplate.Web.Mvc");
            if (Directory.Exists(webMvcFolder)) return webMvcFolder;

            var webHostFolder = Path.Combine(directoryInfo.FullName,
                $"src{Path.DirectorySeparatorChar}MyCompanyName.AbpZeroTemplate.Web.Host");
