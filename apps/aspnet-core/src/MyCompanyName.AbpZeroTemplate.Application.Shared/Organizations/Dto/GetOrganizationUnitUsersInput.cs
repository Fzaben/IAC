...
namespace MyCompanyName.AbpZeroTemplate.Organizations.Dto
{
    public class GetOrganizationUnitUsersInput : PagedAndSortedInputDto, IShouldNormalize
    {
        ...
        {
            if (string.IsNullOrEmpty(Sorting)) Sorting = "user.Name, user.Surname";

            Sorting = DtoSortingHelper.ReplaceSorting(Sorting, s =>
            {
                if (s.Contains("userName")) s = s.Replace("userName", "user.userName");

                if (s.Contains("addedTime")) s = s.Replace("addedTime", "ouUser.creationTime");
