...
namespace MyCompanyName.AbpZeroTemplate.Core.Extensions
{
    public static class ContextExtensions
    {
        ...
        /// <returns></returns>
        public static bool HasSelectionField(this IResolveFieldContext context, string fieldSelector,
            char namespaceSeperator = ':')
        {
            if (string.IsNullOrWhiteSpace(fieldSelector)) return false;

            if (context.SubFields == null) return false;

            var fragments = fieldSelector.Split(new[] {namespaceSeperator}, StringSplitOptions.RemoveEmptyEntries);

            if (fragments.Length == 1) return context.SubFields.ContainsKey(fragments[0]);

            if (context.SubFields[fragments[0]] == null) return false;

            if (context.SubFields[fragments[0]].SelectionSet == null) return false;

            if (context.SubFields[fragments[0]].SelectionSet.Selections == null) return false;
            ...
            for (var i = 1; i < fragments.Length; i++)
            {
                ...
                if (i == fragments.Length - 1) return true;
