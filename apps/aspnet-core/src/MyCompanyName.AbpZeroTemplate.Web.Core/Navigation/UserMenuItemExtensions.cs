...
namespace MyCompanyName.AbpZeroTemplate.Web.Navigation
{
    public static class UserMenuItemExtensions
    {
        ...
        {
            if (menuItem.Name == currentPageName) return true;
        ...
        {
            if (string.IsNullOrEmpty(menuItem.Url)) return applicationPath;

            if (UrlChecker.IsRooted(menuItem.Url)) return menuItem.Url;
